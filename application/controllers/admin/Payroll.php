   ...
   public function set_salary_details($id = NULL)
{
    ...
    if (!empty($created) || (!empty($edited) && !empty($id))) {
        ...
        $this->payroll_model->_table_name = "tbl_salary_template";
        $this->payroll_model->_primary_key = "salary_template_id";
        $salary_template_id = $this->payroll_model->save($template_data, $id);
if (empty($salary_template_id) && !empty($id)) {
    $salary_template_id = $id; // fallback for updates
}
$salary_template_id = $this->payroll_model->save($template_data, $id);
if (empty($salary_template_id) && !empty($id)) {
    $salary_template_id = $id; // fallback for updates
        ...
        $house_rent_allowance_amount = $this->input->post('house_rent_allowance_amount', TRUE);
        $house_rent_allowance_type = $this->input->post('house_rent_allowance_type', TRUE);
        ...
        if (!empty($house_rent_allowance)) {
            ...
            $asalary_allowance_data['allowance_value'][] = $house_rent_allowance_amount;
            $asalary_allowance_data['allowance_percent'][] = $house_rent_allowance;
            $asalary_allowance_data['allowance_type'][] = $house_rent_allowance_type;
        }
        ...
        if (!empty($asalary_allowance_data['allowance_label'])) {
            foreach ($asalary_allowance_data['allowance_label'] as $hkey => $h_salary_allowance_label) {
                ...
                $alsalary_allowance_data['allowance_percent'] = $asalary_allowance_data['allowance_percent'][$hkey];
                $alsalary_allowance_data['allowance_type'] = $asalary_allowance_data['allowance_type'][$hkey];

                $this->payroll_model->_table_name = "tbl_salary_allowance";
                $this->payroll_model->_primary_key = "salary_allowance_id";
        ...
        $salary_allowance_percent = $this->input->post('allowance_percent', TRUE);
        $salary_allowance_type = $this->input->post('allowance_type', TRUE);
        $salary_allowance_id = $this->input->post('salary_allowance_id', TRUE);

        $existing_allowances = get_any_field('tbl_salary_allowance', array('salary_template_id' => $salary_template_id), 'salary_allowance_id', true);
        if (!empty($existing_allowances)) {
            $existing_allowances = array_column($existing_allowances, 'salary_allowance_id');
            if (!empty($existing_allowances) && !empty($salary_allowance_id)) {
                $to_delete = array_diff($existing_allowances, $salary_allowance_id);
                foreach ($to_delete as $deleted_id) {
                    $this->payroll_model->_table_name = "tbl_salary_allowance";
                    $this->payroll_model->_primary_key = "salary_allowance_id";
        ...
        if (!empty($salary_allowance_label)) {
            foreach ($salary_allowance_label as $key => $label) {
                if (!empty($salary_allowance_value[$key])) {
                    $data = array(
                        'salary_template_id' => $salary_template_id,
                        'allowance_label' => $label,
                        'allowance_value' => $salary_allowance_value[$key],
                        'allowance_percent' => $salary_allowance_percent[$key],
                        'allowance_type' => $salary_allowance_type[$key]
                    );
                    $this->payroll_model->_table_name = "tbl_salary_allowance";
                    $this->payroll_model->_primary_key = "salary_allowance_id";
                    $this->payroll_model->save($data, !empty($salary_allowance_id[$key]) ? $salary_allowance_id[$key] : NULL);
                }
            }
        }
        ...
        $provident_fund_amount = $this->input->post('provident_fund_amount', TRUE);
        $provident_fund_type = $this->input->post('provident_fund_type', TRUE);

        $tax_deduction = $this->input->post('tax_deduction', TRUE);
        $tax_deduction_amount = $this->input->post('tax_deduction_amount', TRUE);
        $tax_deduction_type = $this->input->post('tax_deduction_type', TRUE);

        $default_deductions = [];
        if (!empty($provident_fund)) {
            $default_deductions[] = array(
                'deduction_label' => lang('provident_fund'),
                'deduction_value' => $provident_fund_amount,
                'deduction_type' => $provident_fund_type,
                'deduction_percent' => $provident_fund
            );
        }
        if (!empty($tax_deduction)) {
            $default_deductions[] = array(
                'deduction_label' => lang('tax_deduction'),
                'deduction_value' => $tax_deduction_amount,
                'deduction_type' => $tax_deduction_type,
                'deduction_percent' => $tax_deduction
            );
        }

        foreach ($default_deductions as $deduction) {
            $deduction['salary_template_id'] = $salary_template_id;
            $this->payroll_model->_table_name = "tbl_salary_deduction";
            $this->payroll_model->_primary_key = "salary_deduction_id";
            $this->payroll_model->save($deduction);
        }

        // Dynamic deductions
        $deduction_label = $this->input->post('deduction_label', TRUE);
        $deduction_value = $this->input->post('deduction_value', TRUE);
        $deduction_percent = $this->input->post('deduction_percent', TRUE);
        $deduction_type = $this->input->post('deduction_type', TRUE);
        $salary_deduction_id = $this->input->post('salary_deduction_id', TRUE);

        $existing_deductions = get_any_field('tbl_salary_deduction', array('salary_template_id' => $salary_template_id), 'salary_deduction_id', true);
        if (!empty($existing_deductions)) {
            $existing_deductions = array_column($existing_deductions, 'salary_deduction_id');
            if (!empty($existing_deductions) && !empty($salary_deduction_id)) {
                $to_delete = array_diff($existing_deductions, $salary_deduction_id);
                foreach ($to_delete as $deleted_id) {
                    $this->payroll_model->_table_name = "tbl_salary_deduction";
                    $this->payroll_model->_primary_key = "salary_deduction_id";
                    $this->payroll_model->delete($deleted_id);
        ...
        if (!empty($deduction_label)) {
            foreach ($deduction_label as $key => $label) {
                if (!empty($deduction_value[$key])) {
                    $data = array(
                        'salary_template_id' => $salary_template_id,
                        'deduction_label' => $label,
                        'deduction_value' => $deduction_value[$key],
                        'deduction_percent' => $deduction_percent[$key],
                        'deduction_type' => $deduction_type[$key]
                    );
                    $this->payroll_model->_table_name = "tbl_salary_deduction";
                    $this->payroll_model->_primary_key = "salary_deduction_id";
                    $this->payroll_model->save($data, !empty($salary_deduction_id[$key]) ? $salary_deduction_id[$key] : NULL);
                }
            }
        }

        $activity = !empty($id) ? 'activity_salary_template_update' : 'activity_salary_template_added';
        $msg = !empty($id) ? lang('salary_template_update') : lang('salary_template_added');
