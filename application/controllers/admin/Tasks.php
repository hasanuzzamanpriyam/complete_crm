...
class Tasks extends Admin_Controller
{
    ...
    function save_imported()
    {
        ...
        foreach ($files as $key => $value) {
            $data['task_name'] = trim($value[0]);
            $data['task_start_date'] = date('Y-m-d', strtotime($value[1]));
            $data['due_date'] = date('Y-m-d', strtotime($value[2]));
            $data['task_hour'] = trim($value[3]);
            $data['task_progress'] = trim($value[4]);
            $data['task_description'] = trim($value[5]);
