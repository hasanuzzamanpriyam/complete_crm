...
class Tickets extends Client_Controller
{
    ...
    public function save_reply($id = NULL)
    {
        $status = $this->uri->segment(6);
        if (!empty($status)) {
            $this->tickets_model->set_action(array('tickets_id' => $id), array('status' => $status), 'tbl_tickets');
    ...
    public function ticketsList($filterBy = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $custom_field = custom_form_table_search(7);
            $action_array = array('tickets_id');
            $main_column = array('ticket_code', 'subject', 'reporter', 'priority', 'tbl_departments.deptname', 'status');
    ...
    function get_notify_ticket_reply($users, $postdata)
    {
        ...
        switch ($users) {
            case 'admin':
                ...
                if (!empty($designation_info)) {
                    foreach ($designation_info as $v_designation) {
                        $details = $this->db->where('designations_id', $v_designation->designations_id)->get('tbl_account_details')->row();
                        if (!empty($details)) {
                            $user_info[] = $details;
                        }
                    }
