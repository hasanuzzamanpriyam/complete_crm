...
<div class="nav-tabs-custom">
    ...
    <div class="tab-content bg-white pt-lg mt-lg">
        ...
        <div class="tab-pane active" id="manage">
            <form role="form" id="form" data-parsley-validate="" novalidate="" enctype="multipart/form-data"
                  action="<?php echo base_url(); ?>admin/accounting/save_payment_voucher/<?= (!empty($payment_voucher->voucher_id) ? $payment_voucher->voucher_id : ''); ?>"
                ...
                <div class="form-group">
                    ...
                    <div class="col-lg-7">
                        ...
                        if (!empty($payment_voucher)) {
                            echo $payment_voucher->reference_no;
                        } else {
                            if (empty(config_item('payment_voucher_format'))) {
                                echo config_item('payment_voucher_prefix');
                            }
                            if (config_item('increment_payment_voucher') == 'FALSE') {
                                $this->load->helper('string');
                                echo random_string('nozero', 6);
                            } else {
                                echo $this->accounting_model->generate_payment_voucher_prefix();
                            }
                        }
                        ?>"
                               name="reference_no">
                    ...
                    <div class="col-lg-7">
                        <div class="input-group">
                            <input required type="text" name="date"
                                   ...
                                   value="<?php
                                   if (!empty($payment_voucher->date)) {
                                       echo $payment_voucher->date;
                                   } else {
                                       echo date('Y-m-d');
                                   }
                                   ?>"
                    ...
                    <div class="col-sm-8">
                        <select name="account_id" class="form-control select_box" required>
                            ...
                            foreach ($accounts as $account_id => $account) {
                                ...
                                ?>" <?php
                                if (!empty($payment_voucher->account_id)) {
                                    if ($payment_voucher->account_id == $account_id) {
                                        echo 'selected';
                                    }
                                }
                                ?>><?= $account ?></option>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                    <textarea name="notes"
                              class="form-control textarea"><?= (!empty($payment_voucher->notes) ? $payment_voucher->notes : ''); ?></textarea>
                    </div>
                </div>
                ...
                <div class="table-responsive s_table mt-lg pt-lg">
                    <table class="table invoice-items-table items">
                        ...
                        <tbody id="payment_voucher_items">
                                ...
                                <tr class="item-row">
                                        ...
                                        <select name="supplier_client_id[]" required
                                            ...
                                            if (!empty($suppliers)) {
                                                foreach ($suppliers as $supplier_id => $supplier) {
                                                    ?>
                                                    <option value="<?= $supplier_id == 0 ? '' : $supplier_id; ?>"
                                                        <?php
                                                        if (!empty($payment_voucher_item->supplier_client_id)) {
                                                            if ($payment_voucher_item->supplier_client_id == $supplier_id) {
                                                                echo 'selected';
                                                            }
                                                        }
                                                        ?>><?= $supplier ?></option>
                                    ...
                                    <td>
                                        <input type="text" name="description[]"
                                               ...
                                               value="<?= (!empty($payment_voucher_item->description) ? $payment_voucher_item->description : ''); ?>">
                                    </td>
                                    <td>
                                        <input type="text" name="amount[]"
                                               class="form-control amount"
                                               value="<?= (!empty($payment_voucher_item->amount) ? $payment_voucher_item->amount : ''); ?>">
                                    </td>
                                    <td>
    ...
    <script type="text/javascript">
        ...
        function calculate_payment_voucher() {
            var total_amount = 0;
        ...
        $(document).on('click', '.add-row', function (e) {
            ...
            const account_td = '<td>' +
                ...
                foreach ($suppliers as $supplier_id => $supplier) {
                ?>
                '<option value="<?= $supplier_id == 0 ? '' : $supplier_id; ?>"><?= $supplier ?></option>' +
                <?php
                }
                }
                ?>
            ...
            const end_tr = '</tr>';
            // append all td
            $('#payment_voucher_items').append(row + serial_number_td + account_td + description_td + amount_td + delete_td + end_tr);
            select2_init();
            calculate_payment_voucher();
