    ...
    <div class="col-sm-12">
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            <span data-placement="top" data-toggle="tooltip" title="<?= lang('generate_invoice') ?>">
                <a data-toggle="modal" data-target="#myModal"
                   ...
                   class="btn btn-info mr-sm"><i class="fa fa-money"></i> <?= lang('invoice') ?>
                </a>
            </span>
        <?php } ?>

        <?php if (timer_status('projects', $project_details->project_id, 'on')) { ?>
            <a data-toggle="tooltip" data-placement="top" title="<?= lang('stop_timer') ?>" class="btn btn-danger mr-sm"
               href="<?= base_url() ?>admin/projects/tasks_timer/off/<?= $project_details->project_id ?>">
               <i class="fa fa-clock-o fa-spin"></i>
            </a>
        <?php } else { ?>
            <a data-toggle="tooltip" data-placement="top" title="<?= lang('start_timer') ?>" class="btn btn-success mr-sm"
               href="<?= base_url() ?>admin/projects/tasks_timer/on/<?= $project_details->project_id ?>">
               <i class="fa fa-clock-o"></i>
            </a>
        <?php } ?>

        <a data-toggle="tooltip" data-placement="top" title="<?= $title ?>"
           href="<?= base_url() ?>admin/projects/<?= $url ?>" class="btn btn-<?= $btn ?> mr-sm">
        ...
        <?php if (!empty($can_edit) && !empty($edited)) { ?>
            <a data-toggle="modal" data-target="#myModal" title="<?= lang('clone_project') ?>"
               ...
               class="btn btn-purple mr-sm"><i class="fa fa-copy"></i>
            </a>
        <?php } ?>
        ...
        <a data-toggle="tooltip" data-placement="top" title="<?= lang('export_report') ?>"
           ...
           class="btn btn-danger mr-sm"><i class="fa fa-file-pdf-o"></i>
        </a>
    </div>
</div>
