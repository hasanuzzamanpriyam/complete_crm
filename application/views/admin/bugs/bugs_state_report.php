...
for ($iDay = 7; $iDay >= 0; $iDay--) {
    ...
    $invoice_result[$date] = count(array($this->db->where($where)->get('tbl_bug')->result()));
}
...
if ($this->session->userdata('user_type') == 1) {
    ...
    if (!empty($all_bugs_info)):foreach ($all_bugs_info as $v_bugs):
        if ($v_bugs->bug_status == 'unconfirmed') {
            $unconfirmed += count(array($v_bugs->bug_id));
        }
        if ($v_bugs->bug_status == 'confirmed') {
            $confirmed += count(array($v_bugs->bug_id));
        }
        if ($v_bugs->bug_status == 'in_progress') {
            $in_progress += count(array($v_bugs->bug_id));
        }
        if ($v_bugs->bug_status == 'resolved') {
            $resolved += count(array($v_bugs->bug_id));
        }
        if ($v_bugs->bug_status == 'verified') {
            $verified += count(array($v_bugs->bug_id));
        }
    endforeach;)
    endif;
    if (!empty($all_bugs_info)) {
        $unconfirmed_width = ($unconfirmed / count(array($all_bugs_info))) * 100;
        $confirmed_width = ($confirmed / count(array($all_bugs_info))) * 100;
        $in_progress_width = ($in_progress / count(array($all_bugs_info))) * 100;
        $resolved_width = ($resolved / count(array($all_bugs_info))) * 100;
        $verified_width = ($verified / count(array($all_bugs_info))) * 100;
    } else {
        $unconfirmed_width = 0;
        $confirmed_width = 0;
        $in_progress_width = 0;
                ...
                <div class="pl-sm pr-sm pb-sm">
                    <strong><a style="font-size: 15px" class="b_status" search-type="<?= ('unconfirmed') ?>"
                        ...
                        <small class="pull-right " style="padding-top: 2px"> <?= $unconfirmed ?>
                            / <?= count(array($all_bugs_info)) ?></small>
                    </strong>
