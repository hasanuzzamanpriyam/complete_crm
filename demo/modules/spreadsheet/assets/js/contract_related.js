    ...
    $(document).on("dblclick", "#spreadsheet-advanced .tr-pointer", function () {
        ...
        if (type == "file") {
            if (share == false) {
                ...
                $.get(admin_url + 'spreadsheet/get_hash_related/' + related_id + '/' + pathname + '/' + id_set).done(function (response) {
                    response = JSON.parse(response);
                    window.location.replace(site_url + 'spreadsheet/spreadsheet_client/file_view_share_related/' + response.hash);
                })
            }
        } else {
