...
class Imap_zisco
{
    ...
    function my_imap_status($stream, $mailbox='', $info=SA_ALL)
    {
        ...
        if( (empty($mailbox)) || ($mailbox==$curr_obj->Mailbox) ) { echo '<pre>'; return $curr_obj;}
     ...
     */
    public function getUnreadMessages($withbody = true, $folder)
    {
        $emails = [];
        $result = imap_search($this->imap, 'UNSEEN');
        if ($result) {
    ...
    public function getMessages($withbody = true, $folder)
    {
        $emails = [];       
        $f = $this->my_imap_status($this->imap, $folder);        
        $count = $f->Nmsgs;
        for ($i = 1 ; $i <= $count; $i++) {
            $emails[] = $this->formatMessage($i, $withbody);
        }
    ...
    public function getAllEmailAddresses()
    {
        ...
        foreach ($this->getFolders() as $folder) {
            ...
            foreach ($this->getMessages(false, $folder) as $message) {
                $emails[] = $message['from'];
